...
function SpectrumChart() {
  ...
  const lockActive = lockRow || lockCol;
  ...
  const reset = () => { setQ(""); setSgOn(SUPERGROUPS.map(s => s.id)); setClsOn(CLASSES.map(c => c.name)); setLockRow(null); setLockCol(null); };
  ...
  const leaveCell = () => { if (!lockActive) { setHoverRow(null); setHoverCol(null); } setTip(null); };
                    ...
                    return (
                      <tr key={a.name} className={rowOn ? "sx-row-on" : ""}>
                        ...
                        {visOrgs.map(o => {
                          ...
                          const dim = lockActive && !((lockRow ? lockRow === a.name : true) && (lockCol ? lockCol === o.id : true));
                          let cn = "sx-cell";
